...
def perfprof(
    ...
    if ppfix:
        data = np.array(data >= ppfixmax, dtype=np.int) * data + np.array(
            data < ppfixmax, dtype=np.int
    ...
    if len(linespecs) != n:
        raise ValueError(
            "Length of argument linespecs must equal "
    ...
    if legendnames is not None:
        if len(legendnames) != n:
            raise ValueError(
                "Length of argument legendnames must "
    ...
    for alt in range(n):  # for each alternative
        ...
        theta = np.unique(col)
        r = len(theta)
        myarray = np.repeat(col, r).reshape(len(col), r) <= np.repeat(
            theta, len(col)
        ).reshape((len(col), r), order="F")
        ...
        if legendnames is None:
            ...
            plt.plot(x, y, linespecs[alt], linewidth=linewidth, label=legendnames[alt])
