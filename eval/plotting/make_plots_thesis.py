...
OUTPUT_PATH = os.path.normpath(
    os.path.join(os.path.dirname(os.path.abspath(__file__)), "../thesis/")
)
...
def run_avg_all_times(problem, graph):
    ...
    avg_all.to_csv(
        os.path.join(OUTPUT_PATH, "thesis_avg_all_latex_read.csv"), index=False
    )
...
def run_avg_fast_times(problem, graph):
    ...
    avg_all = avg_all.groupby(["algo"]).mean()
    avg_all.to_csv(
        os.path.join(OUTPUT_PATH, "thesis_avg_fast_latex_read.csv"), index=False
    )
