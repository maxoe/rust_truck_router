...
use rust_truck_router::{
    algo::{csp_2_core_ch::CSP2CoreContractionHierarchy, dijkstra::Dijkstra},
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let mut search = CSP2CoreContractionHierarchy::load_from_routingkit_dir(path.join("core_ch"))?;
    search.check();
    ...
    let file = File::create("measure_core_ch_csp_2_1000_queries_rank_times-".to_owned() + path.file_name().unwrap().to_str().unwrap() + ".txt")?;
